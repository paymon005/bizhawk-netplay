...
- **The repair budget outruns the tick budget.** `RepairBudgetFrames` allows 33.4 ms while `TickBudgetMs()` is ~28.4 ms, so the solver can permit a repair the frame tick has no room for. Sparse keyframes brings a worst-case repair to ~27 ms and inside it for the first time, but the two constants should agree by construction rather than by luck.
