...
- **Sync-settings check is best-effort** — the handshake compares the core's real sync-settings blob (read via `ISettable.GetSyncSettings()`), so mismatched per-core settings (e.g. different N64 video plugins) are refused up front rather than desyncing. If a core's settings can't be read it falls back to a coarse core+version+system digest, which wouldn't catch such a mismatch.
  Since v0.24.0 the refusal **names the settings**: `VideoPlugin (yours GLideN64, theirs Rice), Region (yours NTSC, theirs PAL)`. Both peers compare symmetrically, so each side's "yours" is its own and neither player has to send the other a log. The hash still makes the decision — the field list only explains it, and where it can't (a peer that sent none, a difference that survives flattening) it says so rather than implying the settings match.
